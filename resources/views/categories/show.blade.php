...
@section('content')
<div class="bg-indigo-50 dark:bg-slate-800/50 border-b border-indigo-100 dark:border-slate-800 py-12 md:py-20 relative overflow-hidden" style="border-top: 4px solid {{ $category->color }}">
    <!-- Category color accent background -->
    <div class="absolute inset-0 opacity-5 dark:opacity-10" style="background-color: {{ $category->color }}"></div>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        <h1 class="text-3xl md:text-5xl font-extrabold text-gray-900 dark:text-white font-serif flex items-center justify-center gap-3">
            <span class="w-4 h-4 md:w-6 md:h-6 rounded-full inline-block shadow-sm" style="background-color: {{ $category->color }}"></span>
            {{ $category->name }}
        </h1>
        ...
        @endif
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-4 font-medium px-4 py-1 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm shadow-sm inline-block rounded-full">{{ $posts->total() }} articles dans cette catégorie</p>
    </div>
</div>
            ...
            <div class="lg:w-2/3">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="col-span-full py-16 text-center bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white">Aucun article dans cette catégorie</h3>
                        <p class="mt-2 text-gray-500 dark:text-gray-400">Revenez plus tard pour de nouveaux contenus.</p>
                        <div class="mt-6">
            ...
            <div class="lg:w-1/3 space-y-8">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-gray-100 dark:border-slate-700 shadow-sm">
                    ...
                    <form action="{{ route('posts.index') }}" method="GET" class="relative">
                        <input type="text" name="q" placeholder="Rechercher..." class="w-full pl-10 pr-4 py-2.5 rounded-lg border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm shadow-sm transition-all">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 overflow-hidden shadow-sm">
                    ...
                    <ul class="divide-y divide-gray-100 dark:divide-slate-700">
                        ...
                        <li>
                            <a href="{{ route('categories.show', $cat) }}" class="flex items-center justify-between p-4 {{ $category->id === $cat->id ? 'bg-indigo-50 dark:bg-indigo-900/20' : 'hover:bg-gray-50 dark:hover:bg-slate-700/50' }} transition-colors group">
                                <span class="flex items-center gap-3 text-sm font-medium {{ $category->id === $cat->id ? 'text-indigo-700 dark:text-indigo-400 font-bold' : 'text-gray-700 dark:text-gray-300 group-hover:text-indigo-600 dark:group-hover:text-indigo-400' }}">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-gray-100 dark:border-slate-700 shadow-sm">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($tags as $t)
                        <a href="{{ route('tags.show', $t) }}" class="inline-block px-3 py-1.5 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 text-sm rounded-lg hover:bg-indigo-100 hover:text-indigo-700 dark:hover:bg-indigo-900/50 dark:hover:text-indigo-300 transition-colors">
                            #{{ $t->name }}
                        </a>
                        @endforeach
                    </div>
