...
<div class="bg-surface-light dark:bg-surface-darker border-b border-gray-100 dark:border-white/5 pt-12 pb-20 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        <h1 class="text-4xl md:text-5xl lg:text-7xl font-extrabold text-gray-900 dark:text-white mb-6 font-serif tracking-tight flex items-center justify-center gap-2">
            <span class="text-primary-400 dark:text-primary-600 font-light opacity-50">#</span>{{ $tag->name }}
        </h1>
        <p class="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto leading-relaxed">
            Consultez tous nos contenus et discussions relatifs au sujet <strong>{{ $tag->name }}</strong>.
        </p>
    </div>
</div>
        ...
        <div class="lg:grid lg:grid-cols-12 lg:gap-x-12">
            ...
            <div class="lg:col-span-8">
                ...
                <div class="flex items-center justify-between mb-10 pb-6 border-b border-gray-100 dark:border-white/5">
                    <h2 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white font-serif">
                        Articles avec ce tag <span class="text-gray-400 dark:text-gray-500 text-lg font-normal ml-2">({{ $posts->total() }})</span>
                    </h2>
                    <a href="{{ route('posts.index') }}" class="text-sm font-semibold text-primary-600 dark:text-primary-400 hover:text-primary-700 transition flex items-center gap-2 group">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="col-span-full text-center py-20 px-6 rounded-3xl border-2 border-dashed border-gray-200 dark:border-white/10">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white font-serif">Aucun article avec ce tag.</h3>
                        <p class="mt-2 text-gray-500 dark:text-gray-400">Essayez de naviguer vers d'autres sujets.</p>
                    </div>
                    @endforelse
                </div>
            ...
            <div class="lg:col-span-4 space-y-10">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($categories as $cat)
                            <a href="{{ route('categories.show', $cat) }}" class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-medium bg-white dark:bg-surface-dark border border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 hover:border-primary-300 hover:text-primary-600 dark:hover:border-primary-500/30 dark:hover:text-primary-400 shadow-sm transition-all group">
                                <span class="w-2 h-2 rounded-full mr-2" style="background-color: {{ $cat->color }}"></span>
                                {{ $cat->name }}
                                <span class="ml-2 text-xs text-gray-400 dark:text-gray-500 group-hover:text-primary-500">({{ $cat->posts_count }})</span>
                            </a>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-gray-50 dark:bg-surface-dark rounded-3xl p-8 border border-gray-100 dark:border-white/5">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($tags as $t)
                            <a href="{{ route('tags.show', $t) }}" class="inline-flex items-center px-3 py-1.5 {{ $tag->id === $t->id ? 'bg-primary-600 text-white shadow-md border-primary-500' : 'bg-white dark:bg-surface-darker border-gray-200 dark:border-white/5 text-gray-600 dark:text-gray-400 hover:border-primary-500 hover:text-primary-600 dark:hover:border-primary-500 dark:hover:text-primary-400' }} text-xs font-medium rounded-lg border transition-colors shadow-sm">
                                #{{ $t->name }}
                            </a>
                        @endforeach
                    </div>
