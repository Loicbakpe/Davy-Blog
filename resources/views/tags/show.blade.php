...
@section('content')
<div class="bg-indigo-50 dark:bg-slate-800/50 border-b border-indigo-100 dark:border-slate-800 py-12 md:py-20 relative overflow-hidden">
    <!-- Tag symbol background -->
    <div class="absolute inset-0 flex items-center justify-center opacity-5 dark:opacity-10 pointer-events-none">
        <span class="text-[20rem] font-bold text-indigo-500 leading-none">#</span>
    </div>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        <h1 class="text-3xl md:text-5xl font-extrabold text-gray-900 dark:text-white font-serif flex items-center justify-center gap-3">
            <span class="text-indigo-500 dark:text-indigo-400">#</span>{{ $tag->name }}
        </h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-6 font-medium px-4 py-1 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm shadow-sm inline-block rounded-full">{{ $posts->total() }} articles associcés à ce tag</p>
    </div>
</div>
            ...
            <div class="lg:w-2/3">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="col-span-full py-16 text-center bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white">Aucun article avec ce tag</h3>
                        <div class="mt-6">
            ...
            <div class="lg:w-1/3 space-y-8">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 overflow-hidden shadow-sm">
                    ...
                    <ul class="divide-y divide-gray-100 dark:divide-slate-700">
                        ...
                        <li>
                            <a href="{{ route('categories.show', $cat) }}" class="flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors group">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-gray-100 dark:border-slate-700 shadow-sm">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($tags as $t)
                        <a href="{{ route('tags.show', $t) }}" class="inline-block px-3 py-1.5 {{ $tag->id === $t->id ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-indigo-100 hover:text-indigo-700 dark:hover:bg-indigo-900/50 dark:hover:text-indigo-300' }} text-sm rounded-lg transition-colors">
                            #{{ $t->name }}
                        </a>
                        @endforeach
                    </div>
