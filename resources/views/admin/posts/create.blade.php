...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700">
    <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
        <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Rédiger un article</h2>
            ...
            <div class="md:col-span-2 space-y-6">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required
                        class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 text-indigo-500 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-lg font-semibold placeholder-gray-400"
                        placeholder="Ex: Comment bien débuter avec Laravel 11">
                    @error('title')
                ...
                <div>
                    ...
                    <textarea name="excerpt" id="excerpt" rows="2" required
                        class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 placeholder-gray-400"
                        placeholder="Un court résumé qui apparaîtra sur la page d'accueil et les recherches...">{{ old('excerpt') }}</textarea>
                    @error('excerpt')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="body" id="body" rows="15" required
                        class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 font-mono text-sm"
                        placeholder="C'est ici que la magie opère... (Markdown supporté)">{{ old('body') }}</textarea>
                    @error('body')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                    <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Pour l'instant, c'est un champ texte simple. L'éditeur riche sera intégré plus tard.</p>
                </div>
            </div>
                ...
                <div class="bg-gray-50 dark:bg-gray-700/30 p-4 rounded-lg border border-gray-100 dark:border-gray-600">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status" id="status" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <option value="published" {{ old('status') == 'published' ? 'selected' : '' }}>Publié (Visible de tous)</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
