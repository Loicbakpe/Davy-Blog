...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 max-w-2xl">
    <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
        <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Éditer: {{ $tag->name }}</h2>
    ...
    <form action="{{ route('admin.tags.update', $tag) }}" method="POST" class="p-6">
        ...
        <div class="mb-6">
            ...
            @enderror
            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Le slug actuel est : <code>{{ $tag->slug }}</code> (il sera mis à jour si vous modifiez le nom).</p>
        </div>
